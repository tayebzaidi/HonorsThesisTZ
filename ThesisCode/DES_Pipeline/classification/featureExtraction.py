...
def get_pywt_wav_coeffs(lightcurve, wav_type, num_levels, num_band_coeffs=10):
    ...
    for i, filt in enumerate(lightcurve):
        ...
        sc_detail_coeffs = np.array(pywt.swt(magnitude, wav_type, level=num_levels))
        #Extract only the detail coefficients from the the levels of wavelet_analysis
        detail_coeffs = sc_detail_coeffs[:,1,:]
        
        all_coeffs[i*num_points:(i+1)*num_points] = detail_coeffs
