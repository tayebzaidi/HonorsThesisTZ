...
library("Bagidis");
library("rjson");
json_file <- "SN2002de_gpsmoothed.json";
...
detail[20:length(detail)] <- 0
basis[,20:dim(basis)[2]] <- 0
...
modelmag_decomp$series <- rebuilt_modelmag
modelmag_decomp$detail <- detail[1:10]
modelmag_decomp$basis <- basis[,1:10]
Bagidis::BUUHWE.plot(modelmag_decomp, Color = FALSE)
